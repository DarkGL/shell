...
plugin_info_t Plugin_info = {
	...
	"http://www.darkgl.amxx.pl/",	// url
	"SHELL",	// logtag, all caps please
...
void DLLHIDDEN shellCmd( void ) {

	FILE * inputCmd	=	popen( CMD_ARGV( 1 ) , "r" );

	char bufferInput[ 512 ];

	if( !inputCmd ){
		LOG_CONSOLE( PLID , "Shell error" );

		return;
	}

	while( fgets( bufferInput , sizeof( bufferInput ) , inputCmd ) != NULL ){
		LOG_CONSOLE( PLID , bufferInput );
	}
		...
		gamedll_funcs_t *pGamedllFuncs) 
{
	...
	REG_SVR_COMMAND( "shell", shellCmd );
